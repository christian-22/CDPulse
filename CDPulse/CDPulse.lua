...
local function Print(msg)
  DEFAULT_CHAT_FRAME:AddMessage("|cff66ccffCDPulse:|r " .. msg)
end
...
local function ReadSpellCooldownPlain(spellID)
  ...
  local ok, a, b = pcall(C_Spell.GetSpellCooldown, spellID)
  ...
  if type(a) == "table" then
    ...
    return PlainNumber(start), PlainNumber(dur)
  end
...
local function GetCooldownEndTimeFromAPI(spellID)
  ...
  local sStart, sDur = ReadSpellCooldownPlain(spellID)
...
local function IsMeaningfulCooldown(dur)
  local threshold = tonumber(CDPulseDB.gcdThreshold) or 1.6
  return type(dur) == "number" and dur > threshold
end
...
local function ProcessPending(reason)
  ...
  for spellID, info in pairs(pending) do
    ...
    if ArmFromAPI(spellID, reason) then
      -- armed
    else
